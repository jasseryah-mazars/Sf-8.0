...
| `python3 tools/validate_quiz.py` | 1292 questions, **0 errors**; 1268 official / 24 out-of-scope; subchapter coverage 157/157 (100%) |
| `python3 tools/check_section_order.py` | 176/176 chapters compliant |
| `python3 tools/lint_php.py` | 382 complete PHP snippets linted against PHP 8.4.19, **0 failures** |
| `python3 tools/audit.py` | 175 official subtopics; **170 PASS**, 5 TO VERIFY, 0 missing |
| `python3 tools/final_audit.py` | 175 chapters found (live count, no hardcoded denominator); question-bank stats match validate_quiz.py |
| `mkdocs build --strict` | **Exit 0**, clean (no warnings beyond the framework's own benign `Theme._vars` deprecation notice, unrelated to this repo's content) |

None of these numbers are hardcoded anywhere in the reporting tools as of
this run — each command above computes its figures live from the current
repository state.
...
| P1-02 Code/config fragment testing | **Partial.** PHP fragments tested for real against PHP 8.4.19 (382/0 failures, pre-existing tooling, re-verified this run). YAML/XML/Twig fragment-level testing via dedicated parsers not built this run. |
