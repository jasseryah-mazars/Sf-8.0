...
_Generated by `tools/gen_traceability_matrix.py` on 2026-08-27, branch `master`, commit `294b27d` (**working tree had uncommitted changes at generation time**), Python 3.11.15. Re-run the tool to refresh; `tools/check_report_freshness.py` flags this report as stale once the repo's HEAD moves past the commit above._
