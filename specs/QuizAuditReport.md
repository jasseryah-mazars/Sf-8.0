...
_Hand-authored narrative report (not machine-regenerated by a single script —_
_its §1/§1b figures come from running `tools/validate_quiz.py` and_
_`tools/check_quiz_duplicates.py` directly). Last edited 2026-08-27, branch_
_`master`, against commit `0f04266` (the commit this edit's own parent —_
_the commit that actually contains this edit is necessarily later than the_
_hash quoted here, same one-commit lag `tools/check_report_freshness.py`_
_documents for machine-stamped reports). If the quiz bank changes after this_
_commit, re-run both commands above and update this file's numbers by hand._
