...
!!! abstract "How to use"
    93 questions spanning every subchapter of **Security**, ordered easy → hard. Answer before revealing each key. For a timed, cross-topic paper use the [Mock Exams](../revision/mock-exam.md).
...
**C'est quoi ?** Cette page est une **banque de 78 questions type QCM** sur Security, avec correction dépliable sous chaque question. Ce n'est pas un cours : c'est un entraînement, à faire après avoir lu le chapitre.
...
**Q21.** How does an impersonating user return to their own account?  <small>_(easy · single)_</small>

- A. Append ?_switch_user=_exit to any URL on the firewall
- B. Visit the dedicated /switch-exit route
- C. Log out and log back in — there is no other way
- D. Append ?_exit_impersonation=1
...
**Q22.** Every voter abstains and no access_decision_manager option was changed. What is the decision?  <small>_(easy · single)_</small>
...
**Q23.** In Symfony 8, where are a Passport's badges validated?  <small>_(medium · internals)_</small>
...
**Q24.** A stateless firewall (stateless: true) does NOT do which of these?  <small>_(medium · internals)_</small>
...
**Q25.** An authenticator's supports() returns null. What does that mean?  <small>_(medium · trap)_</small>
...
**Q26.** A controller runs $security->getUser()->getUserIdentifier() and fatals on some requests. What is the cause?  <small>_(medium · debug)_</small>
...
**Q27.** A #[IsGranted] check fails for an unauthenticated user. What happens?  <small>_(medium · internals)_</small>
...
**Q28.** In #[IsGranted('EDIT', subject: 'post')], what does subject: 'post' do?  <small>_(medium · code)_</small>
...
**Q29.** Which statement about access_control vs isGranted() is correct?  <small>_(medium · trap)_</small>
...
**Q30.** Which of these funnel through AuthorizationCheckerInterface::isGranted()? (choose 3)  <small>_(medium · multiple)_</small>
...
**Q31.** Two providers are defined and a firewall omits the provider key. Result?  <small>_(medium · trap)_</small>
...
**Q32.** Why must the dev firewall (security: false) be listed first?  <small>_(medium · internals)_</small>
...
**Q33.** Requests under /api are being handled by the session-based main firewall instead of the stateless api firewall. What is wrong?  <small>_(medium · debug)_</small>
...
**Q34.** How do you configure eraseCredentials behaviour in security.yaml in Symfony 8?  <small>_(medium · trap)_</small>
...
**Q35.** When is refreshUser() called?  <small>_(medium · internals)_</small>
...
**Q36.** Does a user provider verify the user's password?  <small>_(medium · trap)_</small>
...
**Q37.** A custom provider does `return $repo->find($id);` in loadUserByIdentifier() and sometimes throws a TypeError. What is the fix?  <small>_(medium · debug)_</small>
...
**Q38.** A provider is configured as `all_users: { chain: { providers: [in_memory, api_users] } }`. What does it do?  <small>_(medium · config)_</small>
...
**Q39.** What does security: false on a firewall do?  <small>_(medium · internals)_</small>
...
**Q40.** Two firewalls should share a logged-in session. What do you configure?  <small>_(medium · single)_</small>
...
**Q41.** What does lazy: true achieve on a firewall?  <small>_(medium · internals)_</small>
...
**Q42.** A firewall configured with `pattern: /api` unexpectedly also intercepts /my-api-docs. Why?  <small>_(medium · debug)_</small>
...
**Q43.** Given firewalls declared as main (no pattern) then api (pattern: ^/api), what happens to /api requests?  <small>_(medium · trap)_</small>
...
**Q44.** How do you keep a user's password out of the session in Symfony 8?  <small>_(medium · code)_</small>
...
**Q45.** Which interface declares getPassword()?  <small>_(medium · trap)_</small>
...
**Q46.** A token-only API user declares getPassword(): string. Login works, but the security layer throws a TypeError when serialising the user. Why?  <small>_(medium · debug)_</small>
...
**Q47.** Where is a login password actually verified?  <small>_(medium · internals)_</small>
...
**Q48.** A hasher entry is `algorithm: sodium, migrate_from: ['bcrypt']`. What does it mean?  <small>_(medium · config)_</small>
...
**Q49.** Which special attribute is broader?  <small>_(medium · trap)_</small>
...
**Q50.** An attribute 'EDITOR' (no ROLE_ prefix) is checked. How does RoleVoter treat it?  <small>_(medium · single)_</small>
...
**Q51.** Given role_hierarchy ROLE_ADMIN: [ROLE_USER], a user with ROLE_ADMIN…  <small>_(medium · internals)_</small>
...
**Q52.** With role_hierarchy ROLE_ADMIN: [ROLE_USER] and ROLE_SUPER_ADMIN: [ROLE_ADMIN], which roles does a ROLE_SUPER_ADMIN user satisfy?  <small>_(medium · config)_</small>
...
**Q53.** Which attribute protects a 'change email' action so that a remember-me cookie is not sufficient?  <small>_(medium · scenario)_</small>
...
**Q54.** A rule has roles: [ROLE_A, ROLE_B]. Access is granted when the user has…  <small>_(medium · single)_</small>
...
**Q55.** No access_control rule matches the request. What happens?  <small>_(medium · trap)_</small>
...
**Q56.** What does requires_channel: https do, and when?  <small>_(medium · internals)_</small>
...
**Q57.** After adding { path: ^/, roles: PUBLIC_ACCESS } at the top of access_control, the ^/admin rule below stops protecting admin. Why?  <small>_(medium · debug)_</small>
...
**Q58.** Which passport type suits a valid-API-token flow with no password to verify?  <small>_(medium · scenario)_</small>
...
**Q59.** In which namespace does PasswordCredentials live?  <small>_(medium · trap)_</small>
...
**Q60.** Which of these live in Passport\Badge (i.e. are badges, not credentials)? (choose 3)  <small>_(medium · multiple)_</small>
...
**Q61.** What does AbstractLoginFormAuthenticator provide that AbstractAuthenticator does not?  <small>_(medium · internals)_</small>
...
**Q62.** The base Voter's supports() returns false. What is the resulting vote?  <small>_(medium · internals)_</small>
...
**Q63.** All voters abstain and allow_if_all_abstain keeps its default. Result?  <small>_(medium · trap)_</small>
...
**Q64.** In a Voter, voteOnAttribute() starts with `if (!$user instanceof AppUser) { return false; }`. Why return false rather than abstain?  <small>_(medium · code)_</small>
...
**Q65.** With access_decision_manager.strategy set to priority, how is the decision made?  <small>_(medium · config)_</small>
...
**Q66.** Which statements about switch_user impersonation are correct in Symfony 8? (multiple)  <small>_(medium · multiple)_</small>

- A. The default required role is ROLE_ALLOWED_TO_SWITCH
- B. While switched, the original token is preserved inside a SwitchUserToken
- C. SwitchUserEvent is dispatched on both switch and exit
- D. switch_user is a global option, not a per-firewall one
- E. While switched, isGranted() still uses the admin's roles
...
**Q67.** To show an 'exit impersonation' banner, a template checks is_granted('ROLE_PREVIOUS_ADMIN'). What is the Symfony 8 verdict?  <small>_(medium · trap)_</small>

- A. Use is_granted('IS_IMPERSONATOR') — ROLE_PREVIOUS_ADMIN is the legacy spelling
- B. It is the recommended check in Symfony 8
- C. Both are wrong; check app.user.impersonating instead
- D. The check must be is_granted('ROLE_ALLOWED_TO_SWITCH')
...
**Q68.** What must a service referenced by login_throttling.limiter implement?  <small>_(medium · single)_</small>

- A. Symfony\Component\HttpFoundation\RateLimiter\RequestRateLimiterInterface
- B. Symfony\Component\RateLimiter\LimiterInterface
- C. AuthenticatorInterface with a supports() returning null
- D. Nothing — it is the name of a framework.rate_limiter entry
...
**Q69.** True or false: login throttling rejects a throttled attempt only after the password has been verified and found wrong.  <small>_(medium · true-false)_</small>

- A. False — LoginThrottlingListener runs on CheckPassportEvent with high priority, before credentials are checked
- B. True — it hooks LoginFailureEvent after credential verification
...
**Q70.** role_hierarchy maps ROLE_ADMIN: ROLE_USER. A user stores only [ROLE_ADMIN]. Which check FAILS for them?  <small>_(medium · trap)_</small>
...
**Q71.** Which API expands ['ROLE_SUPER_ADMIN'] into every role it implies, transitively?  <small>_(medium · single)_</small>
...
**Q72.** Strategy unanimous: voter A grants, voter B abstains, no voter denies. A colleague claims access is denied because 'not everyone agreed'. Verdict?  <small>_(medium · trap)_</small>

- A. Access is granted — unanimous only forbids denies; abstentions are neutral
- B. Access is denied — every voter must return ACCESS_GRANTED
- C. Access is denied — abstain counts as a deny under unanimous
- D. It depends on allow_if_equal_granted_denied
...
**Q73.** Calling $security->logout() from a controller action that is not the logout form throws a CSRF-related error. Fix?  <small>_(medium · single)_</small>

- A. Call $security->logout(false) — CSRF validation is on by default and must be skipped for non-form flows
- B. Disable CSRF globally in framework.yaml
- C. Dispatch LogoutEvent manually instead
- D. Set logout: ~ on the firewall to remove CSRF everywhere
...
**Q74.** When do the optional parameters of Security::login($user, $authenticatorName, $firewallName) become necessary? (multiple)  <small>_(medium · multiple)_</small>

- A. $authenticatorName when the firewall registers more than one authenticator
- B. $firewallName when logging into a firewall other than the one matching the current request
- C. $authenticatorName always — there is no single-authenticator shortcut
- D. $firewallName whenever form_login is used
...
**Q75.** During a successful login, in which order does the AuthenticatorManager dispatch these events?  <small>_(hard · internals)_</small>
...
**Q76.** On a logged-out request, what does isGranted('ROLE_ADMIN') do internally?  <small>_(hard · internals)_</small>
...
**Q77.** For each firewall, what does SecurityExtension compile at container build time?  <small>_(hard · internals)_</small>
...
**Q78.** An admin deletes a user's account while that user is browsing under a stateful firewall. What happens on the user's next request?  <small>_(hard · scenario)_</small>
...
**Q79.** When does the Firewall listener run in the kernel lifecycle?  <small>_(hard · internals)_</small>
...
**Q80.** isEqualTo() returns false when a stored user is refreshed. Effect?  <small>_(hard · internals)_</small>
...
**Q81.** You need a user's role change to immediately end their existing sessions. What do you implement?  <small>_(hard · scenario)_</small>
...
**Q82.** Transparent password rehash on login requires…  <small>_(hard · internals)_</small>
...
**Q83.** A user with a very long passphrase can change the trailing characters and still log in. Which hasher is in use and why?  <small>_(hard · trap)_</small>
...
**Q84.** You set migrate_from and needsRehash() returns true, yet stored hashes are never upgraded. What is missing?  <small>_(hard · debug)_</small>
...
**Q85.** Can an access_control rule pass the matched entity to a voter as the subject?  <small>_(hard · trap)_</small>
...
**Q86.** A rule sets both roles: ROLE_USER and an allow_if expression. When is access granted?  <small>_(hard · config)_</small>
...
**Q87.** An authenticator adds a badge that no CheckPassportEvent listener resolves. What happens?  <small>_(hard · internals)_</small>
...
**Q88.** Under the unanimous strategy, one voter denies while another grants. Outcome?  <small>_(hard · scenario)_</small>
...
**Q89.** Under the unanimous strategy, a voter returns false from voteOnAttribute() for an attribute it does not actually care about. Effect?  <small>_(hard · trap)_</small>
...
**Q90.** Which statements describe the DEFAULT login_throttling limiter? (multiple)  <small>_(hard · multiple)_</small>
...
**Q91.** Where IS the configured role hierarchy taken into account? (multiple)  <small>_(hard · multiple)_</small>

- A. isGranted()/#[IsGranted] checks via the RoleHierarchyVoter
- B. access_control rules with roles:
- C. Explicit calls to RoleHierarchyInterface::getReachableRoleNames()
- D. $token->getRoleNames()
- E. $user->getRoles()
...
**Q92.** Votes cast: one GRANTED, one DENIED, one ABSTAIN (default flags; the granting voter has highest priority). Which strategies GRANT access? (multiple)  <small>_(hard · multiple)_</small>

- A. affirmative — at least one grant exists
- B. consensus — the 1–1 tie falls back to allow_if_equal_granted_denied (default true)
- C. priority — the first non-abstaining voter granted
- D. unanimous — abstain is neutral so it grants
...
**Q93.** Which statements about Security::login() (SecurityBundle) are correct? (multiple)  <small>_(hard · multiple)_</small>
